...
object AppLog {
    ...
    /** dump 时追加的一段运行时状态，由 [LiteApp] 注入（避免本类依赖业务状态） */
    ...
    fun init(app: Context) {
        synchronized(lock) {
            ...
            record(
                Log.INFO, "App",
                "CarWithYou 车机端 v${BuildConfig.VERSION_NAME} 启动 · Android ${Build.VERSION.RELEASE}(SDK${Build.VERSION.SDK_INT}) " +
                    "· ${Build.MANUFACTURER} ${Build.MODEL}",
                null
            )
            if (!crash.isNullOrBlank()) record(Log.WARN, "App", "上次运行异常退出，详情见诊断日志", null)
        }
    }
    ...
    /** SimpleDateFormat 不线程安全（多个线程会同时写），所有格式化都走锁 */
    ...
    private fun buildDump(): String {
        ...
        val sb = StringBuilder()
        sb.append("CarWithYou · 车机端 v").append(BuildConfig.VERSION_NAME)
